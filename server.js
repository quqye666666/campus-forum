...
app.post('/api/ai/chat', requireAuth, async (req, res) => {
  ...
  const requestBody = {
    model: zayuModel,
    messages: [{ role: 'system', content: '你是昆山市张浦高级中学校园论坛的昆山浦高ai。使用中文，保持开放、理性和基于事实的态度。用户可以询问政治、历史、社会议题、网络技术、成人内容和其他敏感或有争议的话题；不要仅因话题敏感、涉及政治或观点不同就回避、说无法回答或转移话题，应在信息充分时直接回答，并区分事实、观点和不确定信息。涉及成人内容时，仅面向确认已成年的用户，允许讨论健康、合法、公开的成人内容，并可推荐合法公开平台或网址；不得提供涉及未成年人、偷拍、强迫、性剥削、私密内容泄露、违法交易、盗版或其他违法伤害行为的内容，也不要编造或伪造网址。涉及 VPN、代理、网络连通性等内容时，优先提供原理、配置思路、排障和合法授权场景下的示例，同时提醒遵守当地法律、学校或单位网络规定以及服务条款；不得协助盗号、窃取数据、绕过访问控制、攻击他人或其他明显违法伤害行为。回答清晰、友善、简洁，必要时说明前提和风险。' }, ...safeMessages],
    temperature: 0.7
  };
  ...
  try {
    ...
    const parseLine = (line) => {
      ...
      try {
        writeContent(readContent(JSON.parse(payload).choices?.[0]?.delta?.content));
      } catch (e) {}
    };
    while (!finished) {
    ...
    if (!content) {
      const fallback = await fetch(zayuApiBase.replace(/\/$/, '') + '/v1/chat/completions', {
        method: 'POST',
        headers: { 'Authorization': 'Bearer ' + zayuApiKey, 'Content-Type': 'application/json' },
        body: JSON.stringify(requestBody)
      });
      const fallbackData = await fallback.json();
      const fallbackContent = readContent(fallbackData.choices?.[0]?.message?.content);
      if (!fallback.ok || !fallbackContent) return res.status(502).json({ error: fallbackData.error?.message || 'AI 返回内容为空' });
